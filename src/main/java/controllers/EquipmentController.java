...
public class EquipmentController implements Initializable {
    ...
    private boolean dbWarningShown = false;
    ...
    private void onAddEquipment() {
        if (!databaseAvailable) {
            showError("Database is offline. Start WAMP MySQL then click Refresh List.");
            return;
        }
        try {
    ...
    private void refreshTable() {
        ...
        try {
            ...
            setDbDependentControlsEnabled(true);
        } catch (SQLException e) {
            databaseAvailable = false;
            setDbDependentControlsEnabled(false);
            if (!dbWarningShown) {
                showError("Could not load equipment. Start WAMP MySQL service and click Refresh List.\n\nDetails: " + e.getMessage());
                dbWarningShown = true;
            }
        }
    ...
    private void setDbDependentControlsEnabled(boolean enabled) {
        nameField.setDisable(!enabled);
        typeField.setDisable(!enabled);
        statusField.setDisable(!enabled);
        purchaseDatePicker.setDisable(!enabled);
        deleteEquipmentBtn.setDisable(!enabled || equipmentTable.getSelectionModel().getSelectedItem() == null);
        updateEquipmentBtn.setDisable(!enabled || equipmentTable.getSelectionModel().getSelectedItem() == null);
    }
